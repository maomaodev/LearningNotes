...
分治应用：[23. 合并 K 个升序链表](https://leetcode.cn/problems/merge-k-sorted-lists/)
...
介绍：程序反复调用自身即为递归，**递归与分治往往都是伴随出现的**。对具有递归特性的数据结构的问题求解，很自然使用递归思想设计算法，如广义表、二叉树、树和图的遍历操作。在设计递归算法时，**千万不要去纠结每一级调用和返回的细节**，我们应该“信任”递归调用可以处理子问题，**只需关注：整个递归的终止条件、当前这一级递归需要做什么以及需要返回什么给上一级**。
...
public class Recursion {
    ...
    // 337. 打家劫舍 III
  	// 4个孙子能偷的钱 + 爷爷能偷的钱 VS 2个儿子能偷的钱，哪个组合钱多，就当做当前节点能偷的最大数
    ...
    private int dfs(int[][] matrix, int i, int j) {
        ...
        // 往上，下，左，右扩展
        int up = i - 1 >= 0 && matrix[i - 1][j] > matrix[i][j] ? dfs(matrix, i - 1, j) : 0;
        int down = i + 1 < m && matrix[i + 1][j] > matrix[i][j] ? dfs(matrix, i + 1, j) : 0;
        int left = j - 1 >= 0 && matrix[i][j - 1] > matrix[i][j] ? dfs(matrix, i, j - 1) : 0;
        int rigth = j + 1 < n && matrix[i][j + 1] > matrix[i][j] ? dfs(matrix, i, j + 1) : 0;
        // 取四周扩展长度的最大值，并保存在备忘录中
        memory[i][j] = 1 + Math.max(up, Math.max(down, Math.max(left, rigth)));
        return memory[i][j];
    }
}
```
...
1. 问题描述
   ...
   假设有 n 种物品：物品 i 的重量为 w<sub>i</sub>，价格为 v<sub>i</sub>，背包所能承受的最大重量为 W，我们如何选择才能使得物品的总价格最高。有三种类型的背包问题，这里指的是 0-1 背包。类似题目还有[416. 分割等和子集](https://leetcode-cn.com/problems/partition-equal-subset-sum/)、[494. 目标和](https://leetcode-cn.com/problems/target-sum/)、[面试题 08.11. 硬币](https://leetcode-cn.com/problems/coin-lcci/)。

   - 0-1背包：每种物品只能选择 0 个或 1 个
   - 多重背包：限定物品最多能选择的个数
   - 完全背包：每种物品可以选择无限次
...
1. 问题描述
   ...
   给定一个整数数组 nums ，找到一个具有最大和的**连续**子数组（子数组最少包含一个元素），返回其最大和。题目详见 [53. 最大子序和](https://leetcode-cn.com/problems/maximum-subarray/)，类似题目还有[152. 乘积最大子数组](https://leetcode-cn.com/problems/maximum-product-subarray/)、[718. 最长重复子数组](https://leetcode-cn.com/problems/maximum-length-of-repeated-subarray/)、[300. 最长递增子序列](https://leetcode-cn.com/problems/longest-increasing-subsequence/)。
...
2. 代码实现
   ...
   public class MaxSubArray {
       ...
       // 一般涉及到“连续”问题的动态规划，都需要使用“结尾”关键字定义状态
   ...
   public class EditDistance {
       ...
       public static int minDistance4Iteration(String word1, String word2) {
           ...
           //	(2)若A和B的最后一个字母相同，则dp[i][j] = 1 + min{dp[i][j-1], dp[i-1][j], dp[i-1][j-1] - 1}
         	// dp[i-1][j] + 1表示删除A[i]，dp[i][j-1] + 1表示删除B[j]
           // dp[i-1][j-1]表示不做任何操作，dp[i-1][j-1] + 1表示替换A[i]
...
1. 问题描述
   ...
   给你一个字符串 s 和一个字符规律 p，请你来实现一个支持 `'.'` 和 `'*'` 的正则表达式匹配。其中 `'.'` 匹配任意单个字符， `'*'` 匹配零个或多个前面的那一个元素。题目详见[10.  正则表达式匹配](https://leetcode-cn.com/problems/regular-expression-matching/)，类似题目还有[44. 通配符匹配](https://leetcode-cn.com/problems/wildcard-matching/)、[115. 不同的子序列](https://leetcode-cn.com/problems/distinct-subsequences/)、[1143. 最长公共子序列](https://leetcode-cn.com/problems/longest-common-subsequence/)、[1458. 两个子序列的最大点积](https://leetcode.cn/problems/max-dot-product-of-two-subsequences/)。
...
2. 代码实现
   ...
   ```java
   // 买卖股票通用解法：DP（有限自动机求状态转移），188题是一般情况，其余都是它的特殊情况或变种
...
1. 问题描述
   ...
   *n* 皇后问题研究的是如何将 *n* 个皇后放置在 *n*×*n* 的棋盘上，并且使皇后彼此之间不能相互攻击。题目详见 [51. N皇后](https://leetcode-cn.com/problems/n-queens/)，类似题目还有[52. N皇后 II](https://leetcode-cn.com/problems/n-queens-ii/)。
...
1. 问题描述
   ...
   * 数字 1-9 在每一个以粗实线分隔的 3x3 宫内只能出现一次。
   ...
   数独部分空格内已填入了数字，空白格用 '.' 表示。题目详见 [37. 解数独](https://leetcode-cn.com/problems/sudoku-solver/)。
...
2. 代码实现
   ...
   public class Soduku {
       ...
       private boolean backtrack(char[][] board, int row, int col) {
           ...
           // 满足结束条件，最后一个位置[8][8]试探后，会试探[8][9]，执行[9][0]
           if (row == 9) {
               return true;
           }
           // 这个位置数字已给出，不需要试探，直接返回下一个位置的试探结果
           if (board[row][col] != '.') {
       ...
       private boolean isValid(char[][] board, int x, int y, char ch) {
           ...
           // 检查3x3方格是否有重复数字，(m,n)为(x,y)所在方格的左上角坐标
           int m = x / 3 * 3, n = y / 3 * 3;
           for (int i = m; i <= m + 2; i++) {
               for (int j = n; j <= n + 2; j++) {
...
public class BitOperation {
    ...
    public int[] singleNumbers(int[] nums) {
        ...
        int one = 0;
        // 分组操作：两个只出现一次的数字在不同的组中；相同的数字会被分到相同的组中（关键）
        for (int num : nums) {
            if ((num & flag) == 0) {
                one ^= num;
            }
    ...
    // 137. 只出现一次的数字 II
    // 类似设计一个逻辑电路，完成00->01->10->00，即当数字出现3次时置零
    // 两个性质：x & ~x = 0     x & ~0 = x
    // 1. 初始时（00）：a = 0    b = 0
    // 2. 出现第一次时（01）：b = (0^x)&(~0) = x    a = (0^x)&(~x) = 0
    // 3. 出现第二次时（10）：b = (x^x)&(~0) = 0    a = (0^x)&(~0) = x
    // 4. 出现第三次时（00）：b = (0^x)&(~x) = 0    a = (x^x)&(~0) = 0
    ...
    public boolean isPowerOfFour(int n) {
        // n & (n-1)判断是否是2的幂，即有且只有一个位为1，也可使用Integer.bitCount()函数
        if (n <= 0 || (n & (n - 1)) != 0) {
            return false;
        }
        // 注意：运算符==比&优先级高
...
介绍：求 x<sup>n</sup> 最简单的方法是通过循环将 n 个 x 乘起来，依次求 x<sup>1</sup>, x<sup>2</sup>, ..., x<sup>n</sup>，时间复杂度为 O(n)。快速幂通过将指数转化为二进制来加快幂运算，**时间复杂度可降至 O(logn)**。以计算 2<sup>31</sup> 为例：
...
应用：[204. 计数质数](https://leetcode-cn.com/problems/count-primes/)
...
应用：[415. 字符串相加](https://leetcode-cn.com/problems/add-strings/)、[43. 字符串相乘](https://leetcode-cn.com/problems/multiply-strings/)
...
应用：[224. 基本计算器](https://leetcode-cn.com/problems/basic-calculator/)、[227. 基本计算器 II](https://leetcode-cn.com/problems/basic-calculator-ii/)、[772. 基本计算器 III](https://leetcode-cn.com/problems/basic-calculator-iii/)

参考：[labuladong的算法小抄](https://www.cnblogs.com/labuladong/p/13975106.html)

<img alt="基本计算器" src="./images/基本计算器.png" height="300px"/>

```java
// 772. 基本计算器 III，包含加减乘除和括号，224题没有乘除运算，227题没有括号
public int calculate(String s) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        // 如果是括号，去掉最外层括号，递归处理，去掉该判断可直接解决227题
        if (ch == '(') {
            int j = i + 1;
            // 待匹配左括号的个数
            int count = 1;
        ...
        // 如果是运算符或到达字符串末尾，处理“前一个”运算符，这里去掉乘除，可直接解决224题
        // 加减号直接入栈；乘除号优先计算，与前一个数字做运算后，将结果入栈
...
应用：[146. LRU缓存机制](https://leetcode-cn.com/problems/lru-cache/)
...
1. 两个整数 int 值的加法/乘法、整数取反等操作都可能导致**越界错误**，计算过程尽量使用 long、double 等类型。
1. 由于答案可能会非常大，请你返回答案对 `10^9 + 7` 取余的结果。**临时结果、模（`10^9 + 7`）都需要定义为 long 类型**，返回结果前先取余，再强制转为 int 类型（正确做法：`(int) (res % 1000000007)`，错误做法：`(int) res % 1000000007`），否则中间计算过程可能越界。
1. **整除运算时，使用取余运算 mod**。注意，被除数 sum 为负数时，取模结果 `sum % K` 也为负数，若要纠正为正数，使用：`(sum % K + K) % K`。
1. **根据题目数据量，找对应时间复杂度的算法，一般接受 10^6 左右**。例如，当题目提示 n <= 10^3 时，则时间复杂度为 O(n^2) 的算法一般可以通过；当题目提示 n <= 10^5 时，则时间复杂度为 O(nlogn) 的算法一般可以通过。