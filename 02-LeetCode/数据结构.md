...
介绍：与单调栈类似，单调队列中的元素从头到尾满足单调性。以递增队列为例，如果新来元素大于（等于）队尾元素则直接入队，否则从队尾开始依次将比新来元素大的元素出队，以此来保证单调性。与优先队列不同，单调队列是线性结构，靠删除元素来保持单调性；而优先队列（二叉堆）是树形结构，相当于自动排序。单调队列（双端队列）常用的场景是：**滑动窗口/区间的最值问题**。
...
```java
    ...
    public int[] maxSlidingWindow(int[] nums, int k) {
        ...
        Deque<Integer> deque = new LinkedList<>();
      	...
      	// 队首弹出是因为滑出窗口范围，队尾弹出是因为新元素更大（窗口范围内，比队尾元素小的不可能是结果）
...
介绍：无法高效获取长度，无法根据偏移快速访问元素，是链表的两个劣势。然而面试的时候经常碰见诸如获取倒数第 k 个元素，获取中间位置的元素，判断链表是否存在环，判断环的长度等和长度与位置有关的问题。这些问题都可以通过灵活运用**快慢指针**来解决。

应用：[141. 环形链表](https://leetcode-cn.com/problems/linked-list-cycle/)、[142. 环形链表 II](https://leetcode-cn.com/problems/linked-list-cycle-ii/)、[202. 快乐数](https://leetcode-cn.com/problems/happy-number/)、[19. 删除链表的倒数第N个节点](https://leetcode-cn.com/problems/remove-nth-node-from-end-of-list/)、[876. 链表的中间结点](https://leetcode-cn.com/problems/middle-of-the-linked-list/)、[109. 有序链表转换二叉搜索树](https://leetcode-cn.com/problems/convert-sorted-list-to-binary-search-tree/)、[234. 回文链表](https://leetcode-cn.com/problems/palindrome-linked-list/)、[287. 寻找重复数](https://leetcode-cn.com/problems/find-the-duplicate-number/)
...
public class FastSlowPointer {
    ...
    // 思路：慢指针每次移动一步，而快指针每次移动两步，每轮移动之后，两者的距离会加一
...
public class ReverseList {
    ...
    public ListNode reverseList4Recursion(ListNode head) {
        ...
        // 关键，在某一时刻：1 -> 2 -> 3 -> 4 <- 5，其中head -> 3，4 -> null，node -> 5
        head.next.next = head;
        head.next = null;
        return node;
    }
...
参考：[leetcode](https://leetcode-cn.com/problems/sort-an-array/solution/fu-xi-ji-chu-pai-xu-suan-fa-java-by-liweiwei1419/)、[菜鸟](https://www.runoob.com/w3cnote/ten-sorting-algorithm.html)

![排序算法对比](./images/排序算法对比.jpg)

### 4.1.1 冒泡排序

介绍：冒泡排序（Bubble Sort）是一种简单的排序算法，它通过重复地遍历待排序的列表，比较相邻的元素并交换它们的位置来实现排序。该算法的名称来源于较小的元素会像"气泡"一样逐渐"浮"到列表的顶端。
...
介绍：快速排序（Quick Sort）是一种高效的排序算法，基于分治的思想。核心思想是通过选择一个基准元素（pivot），将列表分为两部分：一部分小于基准元素，另一部分大于基准元素，然后递归地对这两部分进行排序。
...
```java
    ...
    private void quickSort(int[] nums, int start, int end) {
        if (start < end) {
            ...
            quickSort(nums, start, pivotLoc - 1);    // 对枢纽之前的子序列递归排序
            quickSort(nums, pivotLoc + 1, end);  // 对枢纽之后的子序列递归排序
        }
    }
...
介绍：选择排序（Selection Sort）基本思想是每次从待排序的数据中选择最小（或最大）的元素，放到已排序序列的末尾，直到全部数据排序完成。
...
介绍：堆排序（Heap Sort）是指利用堆设计的一种排序算法。堆是具有以下特性的完全二叉树，其所有非叶子节点均不大于（或不小于）其左右孩子节点，即按完全二叉树的节点编号排列，n 个节点的的关键字序列（k<sub>1</sub>，k<sub>2</sub>，...，k<sub>n</sub>）称为堆，当且仅当满足以下关系：

* 小顶堆：k<sub>i</sub> <= k<sub>2i</sub> 且 k<sub>i</sub> <= k<sub>2i+1</sub>，其中 i = 1，2，...，⌊n/2⌋，在堆排序算法中用于升序排列
* 大顶堆：k<sub>i</sub> >= k<sub>2i</sub> 且 k<sub>i</sub> >= k<sub>2i+1</sub>，其中 i = 1，2，...，⌊n/2⌋，在堆排序算法中用于降序排列
...
```java
    ...
    // 对以pos为根的子树调整为子堆，其中堆有效部分为[0, n]，时间复杂度为O(logn)
...
介绍：插入排序（Insertion Sort）通过构建有序序列，对于未排序的数据，在已排序序列中从后向前（或从前往后）扫描，找到相应位置并插入。
...
介绍：希尔排序（Shell Sort）是插入排序的一种改进版本，也称为缩小增量排序。基本思想是：先将整个待排序的记录序列分割成为若干子序列分别进行直接插入排序，待整个序列中的记录"基本有序"时，再对全体记录进行依次直接插入排序。
...
介绍：归并排序（Merge Sort）是建立在归并操作上的一种有效排序算法，采用分治的思想，将一个大问题分解成若干个小问题，分别解决这些小问题，然后将结果合并起来，最终得到整个问题的解。具体步骤如下，通过不断地分解和合并，最终整个数组将被排序。
...
介绍：计数排序（Counting Sort）是一种非比较型的排序算法，**适用于对整数或有限范围内的数据进行排序**。它的核心思想是统计每个元素的出现次数，然后根据统计结果将元素放回正确的位置。计数排序的时间复杂度为 O(n + k)，其中 n 是待排序元素的数量，k 是数据的范围大小。
...
介绍：基数排序（Radix Sort）是一种非比较型整数排序算法，其原理是将整数按位数切割成不同的数字，然后按每个位数（从低位到高位）分别进行排序。由于整数也可以表达字符串和特定格式的浮点数，所以基数排序不是只能用于整数。基数排序的时间复杂度为 O(n * k)，其中 n 是列表长度，k 是最大数字的位数。具体步骤如下：
...
介绍：桶排序（Bucket Sort）是一种分布式排序算法，它将待排序的元素分配到若干个桶中，然后对每个桶中的元素进行排序（可以使用其他排序算法或递归地使用桶排序），最后将所有桶中的元素按顺序合并。桶排序是计数排序的升级版，计数排序相当于极端情况下每个桶的区间大小是 1 ，而桶排序不是这种极端情况，每个桶的区间大小相等，但依然是遍历元素对号入座（放入对应的区间里）。桶排序的重点就在于，如何规划桶区间的大小和个数，使尽可能少的桶去覆盖所有的元素。
...
介绍：快排本身时间复杂度为 O(nlogn)，但是**用于查找数组中第 k 个最小元素只需要 O(n)**。原因是我们是要找下标为 k 的元素，第一次切分的时候需要遍历整个数组（0 ~ n）找到了下标是 i 的元素，假如 k 比 i 小，那么我们下次切分只需要遍历数组（0 ~ i-1）的元素，反之如果 k 比 i 大，那下次切分只需要遍历数组（k+1 ~ n）的元素。总之可以看作每次划分遍历的元素数目都是上一次遍历的 1/2，因此时间复杂度是 n + n/2 + n/4 + ... + n/n = 2n，因此时间复杂度是 O(n)。另外，**若以任意顺序返回前 K 个元素，则快排同样适用于求数组中前 K 个最大（最小）元素**。
...
介绍：涉及**连续子数组**的问题，我们通常有三种思路：**一是滑动窗口、二是前缀和 + 哈希表/数组，三是动态规划**。假设使用 sum[i] 表示数组 A[] 的前 i 项和，即 `sum[i] = A[0] + A[1] + ... + A[i]`。则连续子数组 **`A[i] + ... + A[j] = sum[j] - sum[i - 1],其中i >= 1`**，为了上式能满足前缀和本身的情况（即 i = 0），我们**人为规定 `sum[-1] = 0`**。特别的，当 j = i 时，数组中的某项 `A[i] = sum[i] - sum[i - 1]`。

应用：[974. 和可被 K 整除的子数组](https://leetcode-cn.com/problems/subarray-sums-divisible-by-k/)、[560. 和为K的子数组](https://leetcode-cn.com/problems/subarray-sum-equals-k/)、[1248. 统计「优美子数组」](https://leetcode-cn.com/problems/count-number-of-nice-subarrays/)、[930. 和相同的二元子数组](https://leetcode-cn.com/problems/binary-subarrays-with-sum/)、[238. 除了自身以外数组的乘积](https://leetcode.cn/problems/product-of-array-except-self/)
...
public class BinarySearch {
    ...
    // 关键是如何判断是否有序：若中间元素 > 最右边元素，则右边无/有序，左边有序（该规律可解决153、154）
...
public class LeftRightPointer {
    ...
    // 上面正向扫描时，如果双指针j、k反向移动，那么两边之和小于第三边时，会有两种情况：
    // 一种是左指针往右移动，一种是右指针往左移动，无法确定。而反向扫描时，只能左指针往右
...
4. 满二叉树和完全二叉树是两种特殊的二叉树，定义如下：

   * 一棵深度为 k 且有 2<sup>k</sup> -1 个结点的二叉树称为满二叉树
   * 约定从根起，从上而下，从左而右，给满二叉树中的每个节点从 1 到 n 连续编号。深度为 k 且含 n 个节点的二叉树，如果其每个节点都与深度为 k 的满二叉树中编号从 1 到 n 的节点一一对应，则称为完全二叉树

   ![满二叉树和完全二叉树](./images/满二叉树和完全二叉树.png)

5. 具有 n 个结点的完全二叉树的深度 k = ⌊ log<sub>2</sub>n ⌋ + 1

6. 对于含 n 个结点的完全二叉树中编号为 i（1 ≤ i ≤ n）的结点：
...
后序遍历应用：[104. 二叉树的最大深度](https://leetcode-cn.com/problems/maximum-depth-of-binary-tree/)、[111. 二叉树的最小深度](https://leetcode-cn.com/problems/minimum-depth-of-binary-tree/)、[114. 二叉树展开为链表](https://leetcode-cn.com/problems/flatten-binary-tree-to-linked-list/)、[124. 二叉树中的最大路径和](https://leetcode-cn.com/problems/binary-tree-maximum-path-sum/)、[236. 二叉树的最近公共祖先](https://leetcode-cn.com/problems/lowest-common-ancestor-of-a-binary-tree/)、[1123. 最深叶节点的最近公共祖先](https://leetcode.cn/problems/lowest-common-ancestor-of-deepest-leaves/)、[968. 监控二叉树](https://leetcode-cn.com/problems/binary-tree-cameras/)
...
public class TraverseTree {
    ...
    public List<Integer> inorderTraversal4Iterator(TreeNode root) {
        ...
        while (!stack.isEmpty()) {
            // 在上次goFarLeft方法中，左中节点按照后进先出的顺序，放置在栈中
    ...
    public List<List<Integer>> levelOrder(TreeNode root) {
        ...
        while (!queue.isEmpty()) {
            List<Integer> l = new LinkedList<>();
          	// 错误用法：for (int count = 0; count < queue.size(); count++)
          	// 因为 queue.size() 在循环中会不断变化，除非暂存 queue.size() 变量
...
public class TopKElements {
    ...
    public List<String> topKFrequent(String[] words, int k) {
        ...
        PriorityQueue<String> heap = new PriorityQueue<>((s1, s2) -> {
            // PriorityQueue将最小元素放在队首，设计Comparator想清楚谁应该被认为更小
            // 方法1：comparator方法返回负，则第一个参数被认为更小（先于第二个）
            // 方法2：s1.compareTo(s2)是自然（升）序；s2.compareTo(s1)是反转（降）序
...
public class MedianFinder {
    ...
    public MedianFinder() {
        ...
        // 大顶堆，建议不要使用λ表达式：(a, b) -> b - a，原因是可能溢出
...
介绍：二叉查找树又称二叉排序树、二叉搜索树，要么是一棵空树，要么是具有如下特性的二叉树：

* **若左子树不空，则左子树上所有节点的值均小于根节点的值**
* **若右子树不空，则右子树上所有节点的值均大于根节点的值**
* 左、右子树也分别是二叉查找树

由定义可知，二叉查找树中节点的值不允许重复，其**中序遍历序列是有序的**。

应用：[96. 不同的二叉搜索树](https://leetcode.cn/problems/unique-binary-search-trees/)、[95. 不同的二叉搜索树 II](https://leetcode.cn/problems/unique-binary-search-trees-ii/)、[98. 验证二叉搜索树](https://leetcode.cn/problems/validate-binary-search-tree/)、[99. 恢复二叉搜索树](https://leetcode.cn/problems/recover-binary-search-tree/)



## 5.4 平衡二叉树

介绍：平衡二叉树有很多种，其中最著名的是高度平衡的 AVL 树。平衡二叉树要么是棵空树，要么是如下特性的**二叉查找树**：它的左子树和右子树都是平衡二叉树，且**左子树和右子树的高度之差的绝对值不超过 1**。

应用：[110. 平衡二叉树](https://leetcode.cn/problems/balanced-binary-tree/)、[1382. 将二叉搜索树变平衡](https://leetcode.cn/problems/balance-a-binary-search-tree/)
...
应用：[990. 等式方程的可满足性](https://leetcode-cn.com/problems/satisfiability-of-equality-equations/)、[684. 冗余连接](https://leetcode-cn.com/problems/redundant-connection/)、[685. 冗余连接 II](https://leetcode-cn.com/problems/redundant-connection-ii/)、[785. 判断二分图](https://leetcode-cn.com/problems/is-graph-bipartite/)、[5548. 最小体力消耗路径](https://leetcode-cn.com/problems/path-with-minimum-effort/)、[1722. 执行交换操作后的最小汉明距离](https://leetcode-cn.com/problems/minimize-hamming-distance-after-swap-operations/)
...
图的遍历是解决图的许多应用的基础，有两种基本方法：深度优先遍历（DFS）类似于树的先序遍历；广度优先遍历（BFS）类似于树的层次遍历。注意，**广度优先遍历（BFS）还可用于求无权图中两顶点之间的最短路径，此时深度优先遍历（DFS）并不适用**。

遍历应用：[130. 被围绕的区域](https://leetcode-cn.com/problems/surrounded-regions/)、[133. 克隆图](https://leetcode-cn.com/problems/clone-graph/)、[547. 省份数量](https://leetcode.cn/problems/number-of-provinces/)、[785. 判断二分图](https://leetcode-cn.com/problems/is-graph-bipartite/)、[886. 可能的二分法](https://leetcode-cn.com/problems/possible-bipartition/)、[1306. 跳跃游戏 III](https://leetcode-cn.com/problems/jump-game-iii/)
...
介绍：在 n 个城市之间假设通信网络，如何实现成本最小化？显然可用连通带权图 G 建模，其中顶点表示城市，边表示线路的成本。图 G 可能存在多棵不同的生成树，其中一定存在一棵总成本最小的生成树，即该生成树上各边的权值之和最小，称为**最小生成树**。常用的最小生成树的构造算法有两种：**普里姆算法（加点法）和克鲁斯卡尔算法（加边法）**。
...
用普里姆算法构造上述带权图的最小生成树时，dist 和 preIndex 数组和集合的变化过程如下：
...
// 普里姆算法，构造最小生成树，并返回最小生成树的权值，若不存在最小生成树，返回-1
// n为顶点数，graph采用邻接数组结构，适合稠密图，时间复杂度O(n^2)
public int prim(int n, int[][] graph) {
    // 最小生成树的总权值
    int totalDist = 0;
    // 初始化距离数组：dist[j]表示当前生成树到顶点j的最小边权
    int[] dist = new int[n];
    // 记录最小生成树的边：preIndex[j]=i表示生成树中顶点i、j相连（不需要时可省略）
    ...
    for (int count = 0; count < n; count++) {
        ...
        for (int j = 0; j < n; j++) {
            // 跳过已加入顶点和不可达边
            if (!visited[j] && graph[k][j] != Integer.MAX_VALUE) {
                // 如果经过k到j的边权更小，则更新
                if (graph[k][j] < dist[j]) {
                    dist[j] = graph[k][j];
                    preIndex[j] = k;
...
// 克鲁斯卡尔算法，构造最小生成树，返回最小生成树的权值，若不存在最小生成树，返回-1
// n为顶点数，edges为边的信息，图采用邻接表结构，适合稀疏图，时间复杂度O(mlogm)，m为边数
// 实际上邻接表应定义为：List<int[]>[] graph，这里直接扁平化边信息，方便处理
...
应用：[743. 网络延迟时间](https://leetcode-cn.com/problems/network-delay-time/)、[1514. 概率最大的路径](https://leetcode-cn.com/problems/path-with-maximum-probability/)、[3650. 边反转的最小路径总成本](https://leetcode.cn/problems/minimum-cost-path-with-edge-reversals/)、[2976. 转换字符串的最小成本 I](https://leetcode.cn/problems/minimum-cost-to-convert-string-i/)
...
public int networkDelayTime(int[][] times, int N, int K) {
    ...
    // 初始化有向带权图的边，顶点位序改为从0开始（对角线可为0或无穷）
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
...
// 朴素迪杰斯特拉算法，求从顶点i到其它顶点的单源最短路径，并返回最短路径的权值数组
// graph采用邻接数组结构，适合稠密图，时间复杂度O(n^2)，n为顶点数，
private int[] dijkstra(int n, int i, int[][] graph) {
    // 初始化距离数组：dist[j]表示从源点i到j的最短距离
    int[] dist = new int[n];
    // 记录最短路径经过的点，可逆向获取源点到各顶点的最短路径（不需要时可省略）
    int[] preIndex = new int[n];
    // 标记顶点是否已找到最短路径
    boolean[] visited = new boolean[n];
    ...
    // 按路径长度升序，依次求源点到其它顶点的最短路径（源点i初始化合并处理）
    for (int count = 0; count < n; count++) {
        // 从未访问顶点中选取最小距离的顶点k
        int k = -1;
        int minDist = Integer.MAX_VALUE;
        ...
        for (int j = 0; j < n; j++) {
            // 跳过已访问顶点和不可达边
            if (!visited[j] && graph[k][j] != Integer.MAX_VALUE) {
                // 经过k再到达j的路径长度小于直接到达j的路径长度，则更新
                int newDist = dist[k] + graph[k][j];
                if (newDist < dist[j]) {
                    dist[j] = newDist;
                    preIndex[j] = k;
                }
...
```java
  ...
  public int networkDelayTime(int[][] times, int N, int K) {
      ...
      Arrays.setAll(graph, edge -> new ArrayList<int[]>());
  ...
  // 堆优化的迪杰斯特拉算法，求从顶点i到其它顶点的单源最短路径，并返回最短路径的权值数组
  // n为顶点数，graph采用邻接表结构，适合稀疏图，时间复杂度O(mlogm)，m为边数
  private int[] dijkstra(int n, int i, List<int[]>[] graph) {
      // 初始化距离数组：dist[j]表示从源点i到j的最短距离
      int[] dist = new int[n];
      // 初始化距离为无穷大，表示不可达
      Arrays.fill(dist, Integer.MAX_VALUE);
      ...
      // 使用优先队列（最小堆）存储：[节点j, 源点i到节点j的最短路长度]
      ...
      while (!minHeap.isEmpty()) {
          ...
          int k = curr[0], distK = curr[1];

          // 如果当前距离大于记录的距离，跳过（处理重复入队的情况）
          if (distK > dist[k]) {
              continue;
          }
          ...
          for (int[] edge : graph[k]) {
              int j = edge[0], weightKJ = edge[1];
              int newDist = distK + weightKJ;

              // 如果找到更短路径，更新距离并加入堆中
              if (newDist < dist[j]) {
                  // 更新 x 的邻居的最短路径
                  dist[j] = newDist;
                  // 懒更新堆：只插入数据，不更新堆中数据，因为PriorityQueue不支持更新操作
                  // 相同节点可能有多个不同的newDist，除了最小的newDist，其余都会触发上面的continue
                  minHeap.offer(new int[] { j, newDist });
              }
          }
      }
...
public int networkDelayTime(int[][] times, int N, int K) {
    ...
    int result = 0;
    int[][] dist = floyd(N, graph);
    for (int j = 0; j < N; j++) {
        if (dist[K - 1][j] == Integer.MAX_VALUE) {
            return -1;
        }
        result = Math.max(result, dist[K - 1][j]);
    }
    return result;
}

// 弗洛伊德算法，求顶点之间的最短路径，返回最短路径的二维权值数组
// n为顶点数，graph采用邻接数组结构，图适合稠密图，时间复杂度O(n^3)
private int[][] floyd(int n, int[][] graph) {
    // 1.状态定义
    // 记录最短路径的权值
    ...
    for (int k = 0; k < n; k++) {
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                // i,j两点之间存在另一点k，使i->k + k->j < i->j（跳过不可达边）
