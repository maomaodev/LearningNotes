...
## 5.1 merge 与 rebase 区别

merge 和 rebase 是合并两个分支的操作，都是 checkout 到某个分支，然后把其它分支合并到该分支。**别的分支不会改变，变的只是自己的分支**。

1. **merge 在无分叉的情况下，会自动使用 fast-forward 方式，快速移动**。站在 master 上，merge future，future 不变，master 快速移动。

   ![merge无分支](./images/git/merge无分支.png)

2. **merge 在有分叉的情况下，会产生一个新的提交点，它就是别的分支多次 commit 的合并**。站在 master 上，merge future，future 不变，master 多出一个 6 的提交，其实是 future 分支 2、3 的合并。也可以站在 future 上，merge master，效果类似。

   ![merge有分支](./images/git/merge有分支.png)

3. **rebase 叫做变基，也是把别的分支合到自己身上，但效果好像在基的基础上重新提交自己的多次 commit**。**在 future 上**，rebase master，master 不变，future 的提交被顶到最后，不会多出一个新的提交点。**但是注意，自己的 commit  ID 实际发生了变化，此时如果 push 到远程已经存在的分支，将会发生冲突（可以尝试使用 `--force-with-lease`）**。**另外，不要在 master 上 rebase future**，因为 rebase 后，master 上的多次提交会被顶到别的分支后面，相当于篡改了历史。

   ![rebase](./images/git/rebase.png)

总结：**merge 的优点是多出一个节点，用于记录从哪拉取的分支，但是失去了顺序。rebase 的优点是多次提交还是在最后，便于回退，但是没有记录从哪拉取的分支**。
...
3. [b站 - Git Merge 和 Rebase 区别](https://www.bilibili.com/video/BV1cv411u7wd/?spm_id_from=333.337.search-card.all.click&vd_source=03ee00a529e3c4f9c2d8c6f412586123)
